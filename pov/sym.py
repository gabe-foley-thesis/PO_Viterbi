...
class TupleStore(dict):
    ...
    def __init__(self, alphas=None, entries=None, sparse=True):
        ...
        assert (
            alphas or entries
        ), "Either alphabets or entries (from which alphabets can be inferred) must be supplied."
        ...
        if alphas == None:
            ...
            self.keylen = len(
                alphas
            )  # length of tuples is the same as the number alphabets
        ...
        for idx in range(self.keylen):
            ...
            if (
                self.alphas != None
            ):  # if specified it needs to be a superset of that we constructed
                if not self.alphas[idx].isSupersetOf(myalpha):
                    raise RuntimeError(
                        "Specified alphabet is not compatible with specified entries"
                    )
    ...
    def __setitem__(self, symkey, value):
        assert self.keylen == len(
            symkey
        ), "All entries in dictionary must be equally long"
    ...
    def getAll(self, symkey=None):
        ...
        for idx in range(self.keylen):
            if symkey[idx] != None:
                if not symkey[idx] in self.alphas[idx]:
                    raise RuntimeError(
                        "Invalid entry: must be symbols from specified alphabet or None"
                    )
...
class TupleEntries(object):
    ...
    def __next__(self):
        ...
        for ndx in self.indices:
            ...
            ):  # if we just entered the last symbol of this alphabet
