...
import numpy
from pov.webservice import *
from pov.sym import *
from pov.prob import *
...
class Alignment:
    ...
    def writeClustal(self, filename=None):
        ...
        for i in range(int(wholeRows)):
            for j in range(len(self.seqs)):
                ...
                string += (
                    self.seqs[j][i * symbolsPerLine : (i + 1) * symbolsPerLine] + "\n"
                )
            string += "\n"
        # Possible last row
    ...
    def displayConsensus(self, theta1=0.2, theta2=0.05, lowercase=True):
        ...
        print(
            (
                "Column\tEntropy\tGaps\tProb\tConserv\tSymbols (Up>=%.2f;Low>=%.2f)\n"
                % (theta1, theta2)
            )
        )
        for col in range(self.alignlen):
    ...
    def saveConsensus(
        ...
        if compact:
            f.write(
                "Column\tConserv\tVariab\tAll (Up>=%.2f;Low>=%.2f)\n" % (theta1, theta2)
            )
        else:
            f.write(
                "Column\tProb\tConserv\tSymbols (Up>=%.2f;Low>=%.2f)\n"
                % (theta1, theta2)
            )
        countrow = 0
        for col in range(self.alignlen):
    ...
    def calcSubstMatrix(self, background=None):
        ...
        seqPairs = (
            numSeqs * (numSeqs - 1)
        ) / 2  # Number of pairs of sequences in ungapped alignment
        aaPairs = (
            seqPairs * columns
    ...
    def outliers(self, cap=None):
        ...
        for r in range(nseqs):
            ...
            for c in range(self.alignlen):
                ...
                logent = (
                    math.log(math.log(agree_cnt, nseqs) + 0.000001)
                    if agree_cnt < cap
                    else 0.0
                )
                if gapmat[r, c] == 1:
    ...
    def calcDistances(self, measure, a=1.0):
        ...
        if not measure in ["fractional", "poisson", "gamma", "jc", "k2p"]:
            raise RuntimeError(
                "Unsupported evolutionary distance measure: %s" % measure
            )
        a = float(a)
        ...
        for i in range(len(self.seqs)):
            for j in range(i + 1, len(self.seqs)):
                ...
                if measure == "fractional":
                    ...
                    dist = -(float(oneless) / alphalen) * math.log(
                        1 - (float(alphalen) / oneless) * p
                    )
                elif measure == "k2p":
                    dist = (
                        (float(oneless) / alphalen)
                        * a
                        * ((1 - (float(alphalen) / oneless) * p) ** (-1 / a) - 1)
                    )
                else:  # measure == 'gamma'
                    dist = a * ((1 - p) ** (-1 / a) - 1)
        ...
        if self.alignlen > 10:
            ...
            for i in range(self.alignlen - 1):
                if (i + 1) % 10 == 0 and (i >= col_start and i < col_end):
                    ...
                    html += (
                        str(i / 10 + 1).split(".")[0][(index * -1) + 1]
                        if (len(str(i / 10 + 1).split(".")[0]) > 1)
                        else "0"
                    )
                elif i >= col_start and i < col_end:
                    html += " "
        ...
        if self.alignlen > 100:
            ...
            for i in range(self.alignlen - 1):
                if (i + 1) % 10 == 0 and i >= 99 and (i >= col_start and i < col_end):
                    index = len(str(i / 10 + 1).split(".")[0])
                    html += (
                        str(i / 10 + 1).split(".")[0][-1]
                        if (len(str(i / 10 + 1).split(".")[0]) > 2)
                        else "0"
                    )
                elif i >= col_start and i < col_end:
                    html += " "
...
def alignGlobal(seqA, seqB, substMatrix, gap=-1):
    ...
    return Alignment(
        [
            Sequence(alignA, seqA.alphabet, seqA.name, gappy=True),
    ...
    for i in range(1, lenA + 1):
        for j in range(1, lenB + 1):
            for k in range(1, lenC + 1):
                ...
                matchABC = (
                    S[i - 1, j - 1, k - 1]
                    + subsMatrix.get(seqA[i - 1], seqB[j - 1])
                ...
                S[i, j, k] = max(
                    [matchABC, matchAB, matchBC, matchAC, gapAB, gapBC, gapAC]
                )
...
class PWM(object):
    ...
    def __init__(self, foreground, background=None, start=0, end=None, pseudo=0.0):
        ...
        if False in [
            col.alpha == self.alphabet for col in foreground[self.start + 1 : self.end]
        ]:
        ...
        if not self.alphabet == self.bg.alpha:
            raise RuntimeError(
                "Background needs to use the same alphabet as the foreground"
            )
        p = self.bg.prob()
...
def getSequence(id, database="uniprotkb", start=None, end=None):
    ...
    try:
        ...
        raise RuntimeError(
            "An error occurred while retrieving the specified sequence: %s (maybe the ID doesn't exist)"
            % id
        )
