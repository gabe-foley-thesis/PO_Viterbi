...
(_sn11, _sn1_, _sn_1, _sn, _sprob) = (
    0,
    0,
    0,
    0,
    0.0,
)  # global variables used by hyper0
    ...
    if not ((n1_i | n_1i | ni) != 0):
        if not (n11i % 10 == 0):
            if n11i == _sn11 + 1:
                _sprob = (
                    _sprob
                    * ((_sn1_ - _sn11) / float(n11i))
                    * ((_sn_1 - _sn11) / float(n11i + _sn - _sn1_ - _sn_1))
                )
                _sn11 = n11i
                return _sprob
            if n11i == _sn11 - 1:
                _sprob = (
                    _sprob
                    * ((_sn11) / float(_sn1_ - n11i))
                    * ((_sn11 + _sn - _sn1_ - _sn_1) / float(_sn_1 - n11i))
                )
                _sn11 = n11i
                return _sprob
        _sn11 = n11i
...
def getRSpval(a, b):
    ...
    sd = math.sqrt(
        (na * nb * (n + 1.0)) / 12.0
    )  # the standard deviation is the same in both sets
